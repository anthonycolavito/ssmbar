#' Calculate Social Security Benefits (Baseline - Current Law)
#'
#' Convenience function that creates one or more stylized workers (and optionally their spouses)
#' and calculates their Social Security benefits through their lifetime using BASELINE
#' (current law) benefit calculations with no reform parameters.
#'
#' This is the baseline version using statutory benefit calculations. For reform-capable
#' benefit calculations that support policy analysis, see \code{\link{calculate_benefits_reform}}.
#'
...
#' }
#'
#' @seealso \code{\link{calculate_benefits_reform}} for the reform-capable version
#'
...
calculate_benefits <- function(birth_yr,
  ...
  # Calculate benefits through the BASELINE pipeline
  # Uses aime(), pia(), cola(), ret(), widow_benefit() from baseline files
  # (no reform parameters)
  worker <- worker %>%
...
# =============================================================================
# REFORM-CAPABLE BENEFIT CALCULATION
# =============================================================================

#' Calculate Social Security Benefits (Reform-Capable)
#'
#' Convenience function that creates one or more stylized workers (and optionally their spouses)
#' and calculates their Social Security benefits through their lifetime using REFORM-CAPABLE
#' benefit calculations that support policy analysis and reform modeling.
#'
#' This is the reform-capable version. For baseline (current law) benefit calculations,
#' see \code{\link{calculate_benefits}}.
#'
#' @param birth_yr Numeric value(s) representing the birth year of the worker(s).
#' @param sex Character value(s) specifying the sex of the worker(s): "male", "female", or "all" (gender-neutral).
#' @param type Character value(s) specifying the earnings level of the worker(s).
...
#'   phase_in_years = 10
#' )
#' reformed_worker <- calculate_benefits_reform(
#'   birth_yr = 1970, sex = "male", type = "medium", age_claim = 67,
#'   factors = sef2025, assumptions = tr2025, reform = my_reform
#' )
#'
#' # Without reform (equivalent to calculate_benefits())
#' baseline_worker <- calculate_benefits_reform(
#'   birth_yr = 1960, sex = "male", type = "medium", age_claim = 67,
#'   factors = sef2025, assumptions = tr2025
#' )
#' }
#'
#' @seealso \code{\link{calculate_benefits}} for the baseline version
#'
#' @importFrom dplyr %>% mutate select filter left_join group_by ungroup arrange case_when if_else
#' @export

calculate_benefits_reform <- function(birth_yr,
                               sex = "all",
                               type,
                               age_claim = 65,
                               factors,
  ...
  # Calculate benefits through the REFORM pipeline
  # Uses aime_reform(), pia_reform(), cola_reform(), ret_reform(), widow_benefit_reform()
  # These functions support reform parameters for policy analysis
  worker <- worker %>%
    aime_reform(assumptions, debugg) %>%
