...
spousal_benefit <- function(worker, debugg = FALSE) {
  ...
  worker <- worker %>%
    group_by(id) %>% arrange(id, age) %>%
    mutate(
      s_rf1_ind = s_rf1[which(age == first(elig_age))], #First spousal reduction factor
      s_rf2_ind = s_rf2[which(age == first(elig_age))], #Second spousal reduction factor
      nra_ind = nra[which(age == first(elig_age))], #NRA for age 62 cohort
      age_at_spouse_claim = age[which(s_age == first(s_claim_age))], #Age when spouse first claims benefits
